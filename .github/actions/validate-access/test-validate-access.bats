...
@test "blocks non-code-owner-non-maintainer user" {
  {
    output=$(${BATS_TEST_DIRNAME}/validate-access.sh "random" "true" $workdir "img" 2>&1)
    exit_status=$?
  } || true
  [[ $exit_status -eq 1 ]]
...
@test "allows code owner" {
  output=$(${BATS_TEST_DIRNAME}/validate-access.sh "code-owner" "true" $workdir "img" 2>&1)
  [[ $(echo "${output}"| tail -n 1) = "The workflow is triggered by code-owner as the code owner" ]]
}

@test "allows image maintainer" {
  output=$(${BATS_TEST_DIRNAME}/validate-access.sh "maintainer" "true" $workdir "img")
...
@test "allows non-code-owner-non-maintainer user" {
  output=$(${BATS_TEST_DIRNAME}/validate-access.sh "random" "false" $workdir "img")
...
@test "teams are expanded to team members" {
  ...
  output=$(${BATS_TEST_DIRNAME}/validate-access.sh "ROCKsBot" "true" $workdir "img")
