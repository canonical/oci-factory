...
if __name__ == "__main__":
    ...
    all_revision_tags = utils.get_all_revision_tags(args.all_revision_tags)
    revision_to_track = utils.get_revision_to_track(all_revision_tags)
    print(
    ...
    all_releases = utils.get_all_releases(args.all_releases)
    tag_mapping_from_all_releases = utils.get_tag_mapping_from_all_releases(
        all_releases
    )
    print(f"Parsing image trigger {args.image_trigger}")
    image_trigger = utils.get_image_trigger(args.image_trigger)
    tag_mapping_from_trigger, all_releases = utils.get_tag_mapping_from_trigger(
    ...
    tag_to_revision = utils.get_tag_to_revision(
        tag_mapping_from_trigger, all_tags_mapping, revision_to_track
    )
    release_tags = utils.get_releases_tags(tag_to_revision)
    group_by_revision = utils.get_group_by_revision(release_tags)
    print(
        "Processed tag aliases and ready to release the following revisions:\n"
        f"{json.dumps(group_by_revision, indent=2)}"
    )
    utils.upload_image(group_by_revision, revision_to_track, args.ghcr_repo, img_name)
    print(
        f"Updating {args.all_releases} file with:\n"
        f"{json.dumps(all_releases, indent=2)}"
    )

    utils.write_release_to_file(args.all_releases, all_releases)
