...
def get_tag_mapping_from_all_releases(all_releases):
    ...
    return tag_mapping_from_all_releases

def get_revision_to_track(all_revision_tags):
    ...
    return revision_to_track

def get_tag_mapping_from_trigger(image_trigger,all_releases):
    ...
    return tag_mapping_from_trigger,all_releases

def get_tag_to_revision(tag_mapping_from_trigger,all_tags_mapping,revision_to_track):
    ...
    return tag_to_revision

def get_releases_tags(tag_to_revision):
...
def get_group_by_revision(release_tags):
...
def upload_image(group_by_revision,revision_to_track,args,img_name):
...
def write_to_file(args,all_releases):
...
def main(args):
    ...
    all_revision_tags=get_all_revision_tags(args)
    revision_to_track=get_revision_to_track(all_revision_tags)
    print(
    ...
    all_releases=get_all_releases(args)
    ...
    image_trigger=get_image_trigger(args)
    tag_mapping_from_trigger,all_releases=get_tag_mapping_from_trigger(image_trigger,all_releases)
    print(
    ...
    tag_to_revision=get_tag_to_revision(tag_mapping_from_trigger,all_tags_mapping,revision_to_track)
    release_tags=get_releases_tags(tag_to_revision)
    group_by_revision=get_group_by_revision(release_tags)
    print(
    "Processed tag aliases and ready to release the following revisions:\n"
    f"{json.dumps(group_by_revision, indent=2)}"
    )
    upload_image(group_by_revision,revision_to_track,args,img_name)
    print(
    f"Updating {args.all_releases} file with:\n"
    f"{json.dumps(all_releases, indent=2)}"
    )

    write_to_file(args,all_releases)
