...
class GenerateOciYaml:
    ...
    def validate_args(self) -> None:
        ...
        if self.repository is None or self.name_of_oci is None or self.image_trigger is None or self.all_releases is None or self.all_revision_tags is None:
            parser.error(
                """One of the required variable is empty"""
            )
    ...
    def main(self) -> None:
        ...
        logging.info(f"Get the tag for {self.name_of_oci}")
        ...
        print(group_by_revision)
        logging.info("Building releases info")
        releases = self.build_releases_data(group_by_revision)
