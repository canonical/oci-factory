...
AVAILABLE_ARCHS="$(yq -r '.releases[].architectures[]' < "${image_doc_folder}"/"${image_doc_filename}".yaml | sort | uniq | paste -sd ',')"
...
mv "${image_doc_folder}/${image_doc_filename}.yaml" "${folder_of_data}"
